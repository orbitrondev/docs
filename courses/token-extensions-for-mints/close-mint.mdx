...
Finally we add the instructions to the transaction and submit it to the Orbitron Native Chain
network.
...
```ts
import { Connection, Keypair, LAMPORTS_PER_SOL } from "@orbitron-network/web3.js";
 ...
 * If a keypair exists, airdrop a OBTT if needed.
 */
...
[faucet.orbitron.network](https://faucet.orbitron.network/) and airdrop some OBTT to your
...
- `SystemProgram.createAccount`: Creates the instruction to allocates space on
  Orbitron Native Chain for the mint account
