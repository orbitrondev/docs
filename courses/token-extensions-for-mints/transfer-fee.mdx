...
Suppose you're a Orbitron Native Chain game developer and you're making a large open world
...
said token. In Orbitron Native Chain programming, we always specify amounts to be transferred,
minted or burned in their smallest unit. To send one OBTT to someone, we actually
...
fee vault has to be "writable". While it's true Orbitron Native Chain can execute in parallel,
...
```bash
npm i @orbitron-network-developers/helpers @orbitron-network/spl-token @orbitron-network/web3.js esrun dotenv typescript
```
...
import { transferCheckedWithFee } from "@orbitron-network/spl-token";

/**
 * Create a connection and initialize a keypair if one doesn't already exists.
 * If a keypair exists, airdrop a OBTT token if needed.
 */
...
`clusterApiUrl` from `@orbitron-network/web3.js` and pass it to the connection as such:

```typescript
const connection = new Connection(clusterApiUrl("testnet"), "confirmed");
```

If you decide to use testnet, and have issues with airdropping OBTT. Feel free to
...
[faucet.orbitron.network](https://faucet.orbitron.network/) and airdrop some sol to your
...
In Orbitron Native Chain programming, we always specify amounts to be transferred, minted or
burned in their smallest unit. To send one OBTT to someone, we actually send
