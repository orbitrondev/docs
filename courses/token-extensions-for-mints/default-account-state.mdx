...
Imagine you're a Orbitron Native Chain game dev, and you only want players of your game to
...
import { Connection, Keypair, PublicKey } from "@orbitron-network/web3.js";
// import { createTokenExtensionMintWithDefaultState } from "./mint-helper"; // This will be uncommented later
import { initializeKeypair, makeKeypairs } from "@orbitron-network-developers/helpers";
...
- `initializeKeypair`: This function creates the keypair for the `payer` and
  also airdrops some OBTT to it
- `makeKeypairs`: This function creates keypairs without airdropping any OBTT
...
The first step in creating a mint is reserving space on Orbitron Native Chain with the
`SystemProgram.createAccount` method. This requires specifying the payer's
keypair, (the account that will fund the creation and provide OBTT for rent
...
OBTT (lamports) necessary to exempt the account from rent and the ID of the token
