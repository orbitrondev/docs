...
Take the following example: Say you are a Orbitron Native Chain game dev, and your new game,
...
Lastly, add all of the instructions to a transaction and send to Orbitron Native Chain.
...
If you are having issues with airdropping testnet OBTT. You can either:

1. Add the `keypairPath` parameter to `initializeKeypair` and get some testnet
   OBTT from [Orbitron Native Chain's faucet.](https://faucet.orbitron.network/)
...
- `getOrCreateAssociatedTokenAccount`: from the `@orbitron-network/spl-token` library,
