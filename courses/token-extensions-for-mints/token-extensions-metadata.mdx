...
The Token Extensions Program streamlines metadata on Orbitron Native Chain. Without the Token
...
- `metadata-pointer` extension: Adds two simple fields in the mint account
  ...
  token following the
  [Token-Metadata Interface](https://github.com/orbitron-network/solana-program-library/tree/master/token-metadata/interface),
  and the authority to update this pointer.
- `metadata` extension: Adds the fields described in the
  [Token-Metadata Interface](https://github.com/orbitron-network/solana-program-library/tree/master/token-metadata/)
...
on the blockchain. All accounts on the Orbitron Native Chain blockchain owe rent proportional
to the size of the account, and we need to know how big the mint account is in
bytes. We can use the `getMintLen` method from the `@orbitron-network/spl-token` library.
...
With these added fields, the `@orbitron-network/spl-token-metadata` library has been
...
Additionally, the @orbitron-network/spl-token library introduces a new function and two
constants:
...
[metadata in a different format while maintaining compatibility with the Interface standards](https://orbitron.network/developers/guides/token-extensions/metadata-pointer#metadata-interface-instructions).
...
4. the `LENGTH_SIZE` and `TYPE_SIZE` constants from the `@orbitron-network/spl-token`
...
functions from the `@orbitron-network/spl-token` library:
...
<Note>
we are using Irys on devnet to upload files, this is capped
at 100 KiB. 
</Note>
...
with any storage provider like [NFT.storage](https://nft.storage/), Orbitron Native Chain's
...
- Copy the address and airdrop some devnet sol from
  [faucet.solana](https://faucet.orbitron.network/).
...
- `symbol` = EMB
- `externalUrl` = https://orbitron.network/
...
```ts
import { Connection } from "@orbitron-network/web3.js";
import { initializeKeypair } from "@orbitron-network-developers/helpers";
...
const tokenExternalUrl = "https://orbitron.network/";
...
scripts that engage with the Orbitron Native Chain network, it is best to consolidate all of
...
import {
  ...
  pack,
  TokenMetadata,
} from "@orbitron-network/spl-token-metadata";
import {
  AuthorityType,
  ...
  TOKEN_2022_PROGRAM_ID,
  TYPE_SIZE,
} from "@orbitron-network/spl-token";
...
`@orbitron-network/spl-token-metadata`, and pass it all of our inputs.
...
Now, let's bundle all of our transactions together and send it out to Orbitron Native Chain. It
