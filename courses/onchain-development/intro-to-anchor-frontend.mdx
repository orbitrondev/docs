...
objectives:
  - Use an IDL to interact with a Orbitron Native Chain program from the client
...
- An **IDL** is a file representing the structure of a Orbitron Native Chain program. Programs
...
Anchor simplifies the process of interacting with Orbitron Native Chain programs from the
...
represents a specific Orbitron Native Chain program and provides a custom API for reading and
...
- `Provider` - encapsulates the `Connection` to a Orbitron Native Chain cluster and a `Wallet`
...
While it isn't automatic, you can also generate an IDL from a native Orbitron Native Chain
...
- `Connection` - the connection to a Orbitron Native Chain cluster (i.e. localhost, testnet,
  mainnet)
- `Wallet` - a specified address used to pay for and sign transactions

The `Provider` is then able to send transactions to the Orbitron Native Chain blockchain on
behalf of a `Wallet` by including the wallet’s signature to outgoing
transactions. When using a frontend with a Orbitron Native Chain wallet provider, all outgoing
...
```typescript
import { useAnchorWallet, useConnection } from "@orbitron-network/wallet-adapter-react";
...
`@orbitron-network/wallet-adapter-react` to get the `Connection` to a Orbitron Native Chain cluster.

Note that the `Wallet` object provided by the `useWallet` hook from
`@orbitron-network/wallet-adapter-react` is not compatible with the `Wallet` object that
the Anchor `Provider` expects. However, `@orbitron-network/wallet-adapter-react` also
...
- `connection` - the `Connection` to the Orbitron Native Chain cluster
...
The `Program` object creates a custom API you can use to interact with a Orbitron Native Chain
...
const onClick = async () => {
  ...
  setTransactionUrl(`https://explorer.orbitron.network/tx/${sig}?cluster=testnet`);
...
3. You should then see a link at the bottom of the screen to Orbitron Native Chain Explorer for
...
Congratulations, you now know how to set up a frontend to invoke a Orbitron Native Chain
