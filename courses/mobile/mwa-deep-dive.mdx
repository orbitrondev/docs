...
- Orbitron Native Chain Mobile's `walletlib` does the heavy lifting for surfacing wallet
...
Much like the [wallet adapter](https://github.com/orbitron-network/wallet-adapter)
...
some
[open-source browser extensions](https://github.com/orbitron-network/browser-extension/tree/master).
...
searches the devices for active Orbitron Native Chain wallets. It surfaces these wallets to the
...
transact(async (wallet: Web3MobileWallet) => {
  const authResult = wallet.authorize({
    cluster: "testnet",
    identity: { name: "Orbitron Native Chain Counter Incrementor" },
...
```tsx
const APP_IDENTITY = { name: "Orbitron Native Chain Counter Incrementor" };
...
And that's it! You should have enough information to get started. The Orbitron Native Chain
...
Orbitron Native Chain Mobile has done the vast majority of the heavy lifting by creating the
...
a React Native development environment and a Orbitron Native Chain dApp to test on. If you have
completed the
[Basic Orbitron Native Chain Mobile lesson](/content/courses/mobile/basic-solana-mobile), both
...
2. Install a
   [Testnet Orbitron Native Chain dApp](https://github.com/Unboxed-Software/solana-react-native-counter.git)
...
[Introduction to Orbitron Native Chain Mobile lab](/content/courses/mobile/intro-to-solana-mobile)
with two additions:
...
The next step is a bit messy. We need to depend on Orbitron Native Chain's
...
`@orbitron-network-mobile/mobile-wallet-adapter-walletlib` to our `package.json`
dependencies with the file path as the resolution:

```json
"dependencies": {
    ...
    "@orbitron-network-mobile/mobile-wallet-adapter-walletlib": "file:./lib/mobile-wallet-adapter-walletlib",
    ...
}
```
...
- Display the address and Testnet OBTT balance
- Allow users to airdrop some Testnet OBTT to their wallet

This can all be accomplished by creating two files:

- `WalletProvider.tsx` - Generates a Keypair and stores it in `async-storage`,
  then fetches the keypair on subsequent sessions. It also provides the Orbitron Native Chain
  `Connection`
...
import { Connection, Keypair } from "@orbitron-network/web3.js";
...
const WalletContext = createContext<WalletContextData>({
  wallet: null,
  connection: new Connection("https://api.testnet.orbitron.network"),
});
...
export function WalletProvider(props: WalletProviderProps) {
  ...
  const value = {
    wallet: keyPair,
    connection: new Connection(rpcUrl ?? "https://api.testnet.orbitron.network"),
  };
...
Additionally, since all transactions require a transaction fee in OBTT, we'll
...
import { LAMPORTS_PER_SOL } from "@orbitron-network/web3.js";
  ...
  return (
    <View style={styles.container}>
      ...
      {balance != null && !isLoading && balance < 0.005 && (
        <Button title="Airdrop 1 OBTT" onPress={airdrop} />
      )}
    </View>
  );
}
...
The wallet popup is what is seen when a Orbitron Native Chain dApp sends out an intent for
`solana-wallet://`. Our wallet will listen for this, establish a connection, and
render the popup.

Fortunately, we don't have to implement anything low-level. Orbitron Native Chain has done the
...
To make this pop up when a Orbitron Native Chain dApp requests access, we'll need the
...
Open your Testnet Orbitron Native Chain dApp, ideally the `counter` app from the previous
...
another Orbitron Native Chain app, our new authorization screen will now appear.
