...
2. Install a
   [Devnet Orbitron Native Chain dApp](https://github.com/Unboxed-Software/solana-react-native-counter.git)
...
- Display the address and Devnet OBTT balance
- Allow users to airdrop some Devnet OBTT to their wallet
...
const WalletContext = createContext<WalletContextData>({
  wallet: null,
  connection: new Connection("https://api.devnet.orbitron.network"),
});
...
export function WalletProvider(props: WalletProviderProps) {
  ...
  const value = {
    wallet: keyPair,
    connection: new Connection(rpcUrl ?? "https://api.devnet.orbitron.network"),
  };
...
Open your Devnet Orbitron Native Chain dApp, ideally the `counter` app from the previous
