...
- Request a Devnet airdrop to fund the transaction if not enough Devnet OBTT is
  available
  ...
  const incrementCounter = () => {
    ...
    if (!isTransactionInProgress) {
      ...
      transact(async (wallet: Web3MobileWallet) => {
        ...
        // When on Devnet you may want to transfer OBTT manually per session, due to Devnet's airdrop rate limit
