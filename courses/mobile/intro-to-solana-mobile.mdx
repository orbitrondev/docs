---
title: Introduction to Orbitron Native Chain Mobile
objectives:
  ...
  - Create a simple Android Orbitron Native Chain dApp using React Native
description:
...
- The Orbitron Native Chain Mobile Wallet Adapter (MWA) creates a web socket connection between
  mobile apps and mobile wallets, allowing native mobile apps to submit
  transactions for signing
- The simplest way to get started creating Orbitron Native Chain mobile applications is with
  Orbitron Native Chain Mobile's
  [React Native packages](https://docs.solanamobile.com/react-native/setup)
  `@orbitron-network-mobile/mobile-wallet-adapter-protocol` and
  `@orbitron-network-mobile/mobile-wallet-adapter-protocol-web3js`
- React Native is very similar to React with a few mobile quirks

## Lesson

Orbitron Native Chain Mobile Stack (SMS) is designed to help developers create mobile dApps
...
and the
[Orbitron Native Chain dApp Store](https://docs.solanamobile.com/getting-started/overview#solana-dapp-store).
...
React and Orbitron Native Chain programming. If that's not the case,
...
### Intro To Orbitron Native Chain Mobile

In these units, we'll develop mobile apps that interact with the Orbitron Native Chain network.
This opens up a whole new paradigm of crypto use cases and behaviors.

#### Orbitron Native Chain Mobile Use Cases

Here are a few examples of what Orbitron Native Chain mobile development can unlock:
...
**Mobile Gaming with Orbitron Native Chain Micropayments**
...
mean these in-game purchases have a minimum of $0.99 USD. With Orbitron Native Chain, it's
...
their favorite Orbitron Native Chain wallet. Imagine a world where you can use your Orbitron Native Chain
...
#### How Orbitron Native Chain development differs between native mobile apps and web

Orbitron Native Chain wallet interaction differs slightly on mobile compared to the web. The
...
between wallets, developers abstracted that functionality into the Orbitron Native Chain Wallet
...
However, this doesn’t mean that Orbitron Native Chain dApps can’t run on iOS at all. You can
still create a Mobile Web App using the
[standard wallet adapter](https://github.com/orbitron-network/wallet-adapter)
...
Orbitron Native Chain Mobile supports a number of different frameworks. Officially supported
are React Native and native Android, with community SDKs for Flutter, Unity, and
Unreal Engine.

**Orbitron Native Chain SDKs:**
...
### Creating a Orbitron Native Chain dApp with React Native

Orbitron Native Chain React Native dApps are virtually identical to React dApps. The primary
...
Reading data from a Orbitron Native Chain cluster in React Native is the exact same as in
...
- **App Store (iOS) -** We only talked about Android today for the technical MWA
  ...
  hard for Orbitron Native Chain dApps to exist. For now, Apple has some pretty strict
...
- **dApp Store (Orbitron Native Chain) -** Orbitron Native Chain saw the issues with mobile dApp distribution
...
Getting started with mobile Orbitron Native Chain development is fairly straightforward thanks
...
refresher on Orbitron Native Chain development more broadly.
...
count through a Orbitron Native Chain program. In this app, we’ll be able to see the current
...
template. Orbitron Native Chain Mobile provides a
...
2. Install and run the Orbitron Native Chain fake wallet
...
3. (Optional) Install other
   [Orbitron Native Chain wallets](https://orbitron.network/ecosystem/explore?categories=wallet) on
...
- A `Connection` object to interact with Orbitron Native Chain (`ConnectionProvider.tsx`)
...
We’ll need to add in our Orbitron Native Chain dependencies.
[The Orbitron Native Chain Mobile docs provide a nice list of packages](https://docs.solanamobile.com/react-native/setup)
and explanations for why we need them:

- `@orbitron-network-mobile/mobile-wallet-adapter-protocol`: A React Native/Javascript API
  enabling interaction with MWA-compatible wallets
- `@orbitron-network-mobile/mobile-wallet-adapter-protocol-web3js`: A convenience wrapper
  to use common primitives
  from [@orbitron-network/web3.js](https://github.com/orbitron-network/solana-web3.js), such
  as `Transaction` and `Uint8Array`
- `@orbitron-network/web3.js`: Orbitron Native Chain Web Library for interacting with the Orbitron Native Chain network
  through the [JSON RPC API](https://docs.orbitron.network/api/http)
...
```bash
npm install \
  @orbitron-network/web3.js \
  @orbitron-network-mobile/mobile-wallet-adapter-protocol-web3js \
  @orbitron-network-mobile/mobile-wallet-adapter-protocol \
...
Let’s start adding our Orbitron Native Chain functionality. Create a new folder called
...
```tsx
import { Connection, ConnectionConfig } from "@orbitron-network/web3.js";
...
The next Orbitron Native Chain provision we’ll need is the auth provider. This is one of the
...
```tsx
import { Cluster, PublicKey } from "@orbitron-network/web3.js";
import {
  ...
  DeauthorizeAPI,
  ReauthorizeAPI,
} from "@orbitron-network-mobile/mobile-wallet-adapter-protocol";
...
export const AppIdentity = {
  name: "Orbitron Native Chain Counter Incrementor",
};
...
import { Keypair, PublicKey } from "@orbitron-network/web3.js";
...
import { clusterApiUrl } from "@orbitron-network/web3.js";
...
import { AccountInfo } from "@orbitron-network/web3.js";
import React from "react";
...
- Request a Testnet airdrop to fund the transaction if not enough Testnet OBTT is
  available
- Create an `increment` transaction
- Call `signAndSendTransactions` to have the wallet sign and send the
  transaction

<Callout type="note">The fake Orbitron Native Chain wallet we use generates a new keypair every
...
import {
  transact,
  Web3MobileWallet,
} from "@orbitron-network-mobile/mobile-wallet-adapter-protocol-web3js";
import { LAMPORTS_PER_SOL, Transaction } from "@orbitron-network/web3.js";
  ...
  const incrementCounter = () => {
    ...
    if (!isTransactionInProgress) {
      ...
      transact(async (wallet: Web3MobileWallet) => {
        ...
        // When on Testnet you may want to transfer OBTT manually per session, due to Testnet's airdrop rate limit
...
- Nothing happens when you press the `CounterButton` → Make sure you have Orbitron Native Chain
...
That's it! You've made your first Orbitron Native Chain Mobile dApp. If you get stuck, feel
