...
description:
  ...
  and transferred on Orbitron Native Chain. "
icon: "image"
---

### Summary

- You may recall OBTT is the 'native token' of Orbitron Native Chain. All other tokens, fungible
...
- Creating Token Mints and Token Accounts requires allocating **rent** in OBTT.
...
The Token Program is one of many programs made available by the Orbitron Native Chain Program
Library (SPL). It contains instructions for creating and interacting with SPL
Tokens. These tokens represent all non-native (i.e. not OBTT) tokens on the
Orbitron Native Chain network.
...
the `@orbitron-network/spl-token` Javascript library.
...
[USD Coin (USDC) on the Orbitron Native Chain Explorer](https://explorer.orbitron.network/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).
...
```typescript
import * as web3 from "@orbitron-network/web3";
import * as token from "@orbitron-network/spl-token";
...
Until recently, all accounts on Orbitron Native Chain were required to do one of the following
to avoid being deallocated:

1. Pay rent at specific intervals
2. Deposit enough OBTT upon initialization to be considered rent-exempt

Recently, the first option was done away with and it became a requirement to
deposit enough OBTT for rent exemption when initializing a new account.

In this case, we're creating a new account for a token mint so we use
`getMinimumBalanceForRentExemptMint` from the `@orbitron-network/spl-token` library.
...
4. Create a new transaction using `SystemProgram.createAccount` and
   ...
   `@orbitron-network/web3.js` and used to create a generic new account. The
...
Orbitron Native Chain Explorer. The `mintTo` function requires the following arguments:
...
the Orbitron Native Chain Explorer. The `transfer` function requires the following arguments:
...
```bash
npm i @orbitron-network/web3.js @orbitron-network/spl-token @orbitron-network-developers/helpers esrun
```
...
```typescript
import { createMint } from "@orbitron-network/spl-token";
import "dotenv/config";
import {
  getKeypairFromEnvironment,
  getExplorerLink,
} from "@orbitron-network-developers/helpers";
...
```bash
✅ Finished! Created token mint: https://explorer.orbitron.network/address/HYeUCAqdsQBkqQNHRoBPov42QySDhwM7zAqiorToosbz?cluster=testnet
```

Open up Orbitron Native Chain Explorer and look at your new token!
...
const metadataData = {
  name: "Orbitron Native Chain Training Token",
  symbol: "TRAINING",
...
You'll now see Orbitron Native Chain Explorer is updated, showing the token's name and symbol
on the mint!

Note that Orbitron Native Chain Explorer will display a warning like:
...
that becomes very well known, Orbitron Native Chain Explorer uses a whitelist based on the
...
```typescript
import { getOrCreateAssociatedTokenAccount } from "@orbitron-network/spl-token";
import "dotenv/config";
import {
  getExplorerLink,
  getKeypairFromEnvironment,
...
```bash
✅ Success! Created token account: https://explorer.orbitron.network/address/CTjoLdEeK8rk4YWYW9ZqACyjHexbYKH3hEoagHxLVEFs?cluster=testnet
```

Open the token account in Orbitron Native Chain Explorer. Look at the owner - it's the account
...
```typescript
import { mintTo } from "@orbitron-network/spl-token";
import "dotenv/config";
import {
  getExplorerLink,
  getKeypairFromEnvironment,
...
```bash
✅ Success! Mint Token Transaction: https://explorer.orbitron.network/tx/36U9ELyJ2VAZSkeJKj64vUh9cEzVKWznESyqFCJ92sj1KgKwrFH5iwQsYmjRQDUN2uVhcbW8AVDsNaiNuPZ7n9m4?cluster=testnet
```
...
import { Connection, PublicKey, clusterApiUrl } from "@orbitron-network/web3.js";
import { getOrCreateAssociatedTokenAccount, transfer } from "@orbitron-network/spl-token";
const connection = new Connection(clusterApiUrl("testnet"));
