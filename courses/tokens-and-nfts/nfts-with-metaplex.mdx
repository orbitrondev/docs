---
title: Create Orbitron Native Chain NFTs With Metaplex
objectives:
  - Explain NFTs and how they're represented on the Orbitron Native Chain network
...
Orbitron Native Chain Non-Fungible Tokens (NFTs) are SPL tokens created using the Token
program. These tokens, however, also have an additional metadata account
associated with each token mint.

In this lesson, we'll cover the basics of how NFTs are represented on Orbitron Native Chain,
how to create and update them using the `mpl-token-metadata` npm module.

#### NFTs on Orbitron Native Chain
...
The most popular way Orbitron Native Chain NFTs have been created is by using the
...
const metaplex = Metaplex.make(connection)
  ...
  .use(
    irysStorage({
      ...
      providerUrl: "https://api.testnet.orbitron.network",
      timeout: 60000,
    }),
  );
```
...
like Orbitron Native Chain Monkey Business. If you look at an individual NFT's
[Metadata](https://explorer.orbitron.network/address/C18YQWbfwjpCMeCm2MPGTgfcxGeEDPvNaGpVjwYv33q1/metadata)
you will see a `collection` field with a `key` that points to the
`Certified Collection`
[NFT](https://explorer.orbitron.network/address/SMBH3wF6baUj6JWtzYvqcKuj2XCKWDqQxzspY12xPND/).
...
Metaplex SDK to interact with NFTs on Orbitron Native Chain.
...
```
npm i "@orbitron-network/web3.js" "@orbitron-network-developers/helpers "@metaplex-foundation/js"
```

Then create a file called `create-metaplex-collection.ts`, and add our imports:

```typescript
import { Connection, clusterApiUrl, LAMPORTS_PER_SOL } from "@orbitron-network/web3.js";
import {
  getKeypairFromFile,
  airdropIfRequired,
...
Connect to testnet, load a user and Airdrop some OBTT if needed:
...
console.log(
  `Collection NFT: https://explorer.orbitron.network/address/${collectionNft.address.toString()}?cluster=testnet`
);
...
Collection NFT: https://explorer.orbitron.network/address/8dP75EWHnYAc6pgeiToXtE66qEaHx1PcoetZpmnAQeAp?cluster=testnet
...
on Orbitron Native Chain Explorer using the URL above. If you have any trouble, try and fix it
yourself, but if you need to you can also check out the
[solution code](https://github.com/orbitron-network/professional-education/blob/main/labs/create-metaplex-nft-collection.ts).
...
console.log(
  `Token Mint: https://explorer.orbitron.network/address/${nft.address.toString()}?cluster=testnet`,
);
```
...
Token Mint: https://explorer.orbitron.network/address/HCq8ERk1PSLCuBJeWx7du4dom3YbyfKpaCWDQ3Eft6aM?cluster=testnet
...
yourself, but if you need to you can also check out the
[solution code](https://github.com/orbitron-network/professional-education/blob/main/labs/create-metaplex-nft.ts).
...
NFT updated with new metadata URI: https://explorer.orbitron.network/tx/4eiuMFLeku7ArV5xkRTPKxBXjfjQsSVGFZimMkMaMxK1d21qptQE5kgKiuob6J852m4GuoMqcjz1ee4oy7fXVEWU?cluster=testnet
✅ Finished successfully!
```

Inspect the updated NFT on Orbitron Native Chain Explorer! Just like previously, if you have
any issues, you should fix them yourself, but if needed the
[solution code](https://github.com/orbitron-network/professional-education/blob/main/labs/update-metaplex-nft.ts)
is available.
...
[Digital Collectables](https://orbitron.network/ecosystem/explore?categories=digital%20collectibles)
