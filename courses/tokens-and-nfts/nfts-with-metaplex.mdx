...
const metaplex = Metaplex.make(connection)
  ...
  .use(
    irysStorage({
      ...
      providerUrl: "https://api.devnet.orbitron.network",
      timeout: 60000,
    }),
  );
```
...
Connect to devnet, load a user and Airdrop some OBTT if needed:
...
console.log(
  `Collection NFT: https://explorer.orbitron.network/address/${collectionNft.address.toString()}?cluster=devnet`
);
...
Collection NFT: https://explorer.orbitron.network/address/8dP75EWHnYAc6pgeiToXtE66qEaHx1PcoetZpmnAQeAp?cluster=devnet
...
console.log(
  `Token Mint: https://explorer.orbitron.network/address/${nft.address.toString()}?cluster=devnet`,
);
```
...
Token Mint: https://explorer.orbitron.network/address/HCq8ERk1PSLCuBJeWx7du4dom3YbyfKpaCWDQ3Eft6aM?cluster=devnet
...
NFT updated with new metadata URI: https://explorer.orbitron.network/tx/4eiuMFLeku7ArV5xkRTPKxBXjfjQsSVGFZimMkMaMxK1d21qptQE5kgKiuob6J852m4GuoMqcjz1ee4oy7fXVEWU?cluster=devnet
✅ Finished successfully!
```
