...
description:
  "How to burn tokens, and approve/revoke token delegations on Orbitron Native Chain."
icon: "image"
---
...
```typescript
import { burn } from "@orbitron-network/spl-token";
```
...
```typescript
import { PublicKey, Transaction } from "@orbitron-network/web3";
import { createBurnInstruction } from "@orbitron-network/spl-token";
...
Orbitron Native Chain Explorer. The `approve` function requires the following arguments:
...
```typescript
import { PublicKey, Transaction } from "@orbitron-network/web3";
import { createApproveInstruction } from "@orbitron-network/spl-token";
...
```typescript
import { revoke } from "@orbitron-network/spl-token";
...
Orbitron Native Chain Explorer. The `revoke` function requires the following arguments:
...
import { Connection, PublicKey, clusterApiUrl } from "@orbitron-network/web3.js";
import { getOrCreateAssociatedTokenAccount, burn } from "@orbitron-network/spl-token";
