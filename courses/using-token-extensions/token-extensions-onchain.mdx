...
The `Token Extensions Program` is a program on Orbitron Native Chain mainnet that provides
additional functionality to Orbitron Native Chain tokens and mints. The
...
accounts. As more and more use cases came to Orbitron Native Chain, there was a need for new
...
This lab will utilize a lot of Anchor and Orbitron Native Chain APIs that have been covered
...
#### 1. Verify Orbitron Native Chain/Anchor/Rust Versions

We will be interacting with the `Token Extension` program in this lab and that
requires you have orbitron cli version ≥ `1.18.0`.
...
If the version printed out after running `orbitron --version` is less than
`1.18.0` then you can update the
[cli version manually](https://docs.orbitronlabs.com/cli/install). Note, at the
time of writing this, you cannot simply run the `orbitron-install update` command.
...
```bash
orbitron-install init 1.18.0
```

If you run into the following error at any point attempting to build the
program, that likely means you do not have the correct version of the Orbitron Native Chain CLI
installed.

```bash
anchor build
error: package `orbitron-program v1.18.0` cannot be built because it requires rustc 1.72.0 or newer, while the currently active rustc version is 1.68.0-dev
Either upgrade to rustc 1.72.0 or newer, or use
cargo update -p orbitron-program@1.18.0 --precise ver
where `ver` is the latest version of `orbitron-program` supporting rustc 1.68.0-dev
```
...
Orbitron Native Chain cli to find out.
...
Orbitron Native Chain programs. Here we define our program ID using the `declare_id` Anchor
...
use {
    anchor_lang::prelude::*,
    orbitron_program::{pubkey::Pubkey},
};
```
...
pub fn transfer_checked<'info>(
    ...
    orbitron_program::program::invoke_signed(
        &ix,
        &[
            ctx.accounts.from,
            ctx.accounts.mint,
