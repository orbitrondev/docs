...
objectives:
  ...
  - Use token extensions from the Orbitron Native Chain CLI
description:
...
- The existing Token Program on Orbitron Native Chain provides interfaces for fungible and
...
is a core Orbitron Native Chain program, adopted by everyone.
...
[16 extensions](https://spl.orbitron.network/token-2022/extensions), four on the Token
...
- **Group pointer** Think of a group of tokens much like a "collection" of
  ...
  [Token-Group Interface](https://github.com/orbitron-network/solana-program-library/tree/master/token-group/interface).

- **Group** This stores the
  [group information](https://github.com/orbitron-network/solana-program-library/tree/master/token-group/interface)
...
the instructions in [this guide](https://spl.orbitron.network/token#setup). After the
...
[guide above](https://spl.orbitron.network/token#setup) as it also describes how to
initialize a local wallet and airdrop OBTT. </Callout>
...
That's it! This is how we can use the Orbitron Native Chain CLI with Token Extension Program to
