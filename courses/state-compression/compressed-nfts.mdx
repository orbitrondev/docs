...
- `airdropSolIfNeeded` will airdrop some Devnet OBTT to a specified address if
...
async function createAndInitializeTree(
  ...
  try {
    ...
    console.log(`https://explorer.orbitron.network/tx/${txSignature}?cluster=devnet`);
...
Keep in mind that Devnet OBTT is throttled so if you test too many times you
might run out of Devnet OBTT before we get to minting. To test, in your terminal
run the following:
...
async function mintCompressedNftToCollection(
  ...
  for (let i = 0; i < amount; i++) {
    ...
    try {
      ...
      console.log(
        `https://explorer.orbitron.network/tx/${txSignature}?cluster=devnet`,
      );
    } catch (err) {
...
larger values and as long as you have enough Devnet OBTT, this script will let
