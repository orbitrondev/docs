...
objectives:
  - Explain the logic flow behind Orbitron Native Chain state compression
  - Explain the difference between a Merkle tree and a concurrent Merkle tree
  - Implement generic state compression in basic Orbitron Native Chain programs
description:
  "How state compression - the tech behind compressed NFTs - works, and how to
  implement it in your own Orbitron Native Chain programs."
icon: "anchor"
---

## Summary

- State Compression on Orbitron Native Chain is most commonly used for compressed NFTs, but
...
written through Orbitron Native Chain programs. You can “trust” the data stored in the accounts
...
The specific data structure used by the Orbitron Native Chain State Compression program is a
...
updating concurrent Merkle trees repeatable and composable throughout the Orbitron Native Chain
...
function from the `@orbitron-network/spl-account-compression` JS package to get the logs,
...
```toml
[test.validator]
url = "https://api.mainnet-beta.orbitron.network"
...
We’ll be using the `@orbitron-network/spl-account-compression` package. Go ahead and
install it:

```bash
yarn add @orbitron-network/spl-account-compression
```
...
import { Connection, PublicKey } from "@orbitron-network/web3.js";
...
`@orbitron-network/web3.js`, `@orbitron-network/spl-account-compression`, and our own utils file.
