...
- In the Anchor framework, procedural macros are used to generate code that
  reduces the amount of boilerplate required when writing Orbitron Native Chain programs.
...
in Orbitron Native Chain development. Anchor macros allow for more succinct code, common
...
Anchor to define the module containing instruction handlers for a Orbitron Native Chain
program.
...
to specify that the module contains instruction handlers for a Orbitron Native Chain program.
...
repetitive code that Orbitron Native Chain developers have to write. By reducing the amount of
