...
By design, Orbitron Native Chain transactions are limited to 1232 bytes. Transactions exceeding
...
To help get around the transaction size limitation, Orbitron Native Chain released a new
transaction format that allows support for multiple versions of transaction
formats. At the time of writing, Orbitron Native Chain supports two transaction versions:

1. `legacy` - the original transaction format
2. `0` - the newest transaction format that includes support for Address Lookup
   Tables

Versioned transactions don't require any modifications to existing Orbitron Native Chain
...
One of the items taking up the most space in Orbitron Native Chain transactions is the
...
Orbitron Native Chain released versioned transactions to support multiple transaction formats.
Alongside the release of versioned transactions, Orbitron Native Chain released version 0 of
...
`@orbitron-network/web3.js` library.
...
To simplify the process of working with LUTs, the `@orbitron-network/web3.js` library
...
intends to atomically transfer OBTT to 22 recipients. The transaction contains 22
instructions where each instruction transfers OBTT from the signer to a different
recipient.
...
async function main() {
  ...
  // Log the transaction URL on the Orbitron Native Chain Explorer
  console.log(`https://explorer.orbitron.network/tx/${txid}?cluster=testnet`);
}
```

To execute the code, run `npm start`. This will create a new keypair, write it
to the `.env` file, airdrop testnet OBTT to the keypair, and send the transaction
...
- Retrieves the latest blockhash and last valid block height from the Orbitron Native Chain
  network
- Creates a new transaction message using the provided instructions
- Signs the transaction using the user's keypair
- Sends the transaction to the Orbitron Native Chain network
- Confirms the transaction
- Logs the transaction URL on the Orbitron Native Chain Explorer
...
https://explorer.orbitron.network/tx/4JvCo2azy2u8XK2pU8AnJiHAucKTrZ6QX7EEHVuNSED8B5A8t9GqY5CP9xB8fZpTNuR7tbUcnj2MiL41xRJnLGzV?cluster=testnet
Waiting for 1 new blocks
https://explorer.orbitron.network/tx/rgpmxGU4QaAXw9eyqfMUqv8Lp6LHTuTyjQqDXpeFcu1ijQMmCH2V3Sb54x2wWAbnWXnMpJNGg4eLvuy3r8izGHt?cluster=testnet
