...
description: Access real-world data inside a Orbitron Native program.
icon: "globe"
---
...
- There are many
  [Oracle providers on Orbitron Native](https://orbitron.network/ecosystem/explore?categories=oracle).
- You can build your own Oracle to create a custom data feed
- You have to be careful when choosing your data feed providers

## Lesson

[Oracles](https://orbitron.network/ecosystem/explore?categories=oracle) are services
...
Orbitron Native, and how to effectively use oracles in your Orbitron Native development.
...
[on their website](https://app.switchboard.xyz/orbitron/testnet/explore)
...
Switchboard oracles store data on Orbitron Native using data feeds. These data feeds,
...
Orbitron Native account managed by the Switchboard program. When an oracle updates, it
writes the data directly to these accounts. Let's go over a few terms to
understand how Switchboard works:

- **[Aggregator (Data Feed)](https://github.com/switchboard-xyz/sbv2-orbitron/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-orbitron/src/oracle_program/accounts/aggregator.rs#L60)** -
  ...
  Aggregator is the account owned by the Switchboard Orbitron Native program and is where
  the data is published onchain.
- **[Job](https://github.com/switchboard-xyz/sbv2-orbitron/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-orbitron/src/oracle_program/accounts/job.rs)** -
...
- **Oracle Queue** - A group of oracles that get assigned to update requests in
  ...
  onchain in an
  [account owned by the Switchboard program](https://github.com/switchboard-xyz/orbitron-sdk/blob/9dc3df8a5abe261e23d46d14f9e80a7032bb346c/javascript/orbitron.js/src/generated/oracle-program/accounts/OracleQueueAccountData.ts#L8).
...
1. Oracle queue setup - When an update is requested from a queue, the next `N`
   ...
   invoking the
   [oracleQueueInit instruction](https://github.com/switchboard-xyz/orbitron-sdk/blob/9dc3df8a5abe261e23d46d14f9e80a7032bb346c/javascript/orbitron.js/src/generated/oracle-program/instructions/oracleQueueInit.ts#L13)
   on the Switchboard Orbitron Native program.
...
To use Switchboard oracles and incorporate off-chain data into a Orbitron Native program,
you first have to find a feed that provides the data you need. Switchboard feeds
are public and there are many
[already available that you can choose from](https://app.switchboard.xyz/orbitron/testnet/explore).
...
[BTC_USD feed](https://app.switchboard.xyz/orbitron/testnet/feed/8SXvChNYFhRq4EZuZvnhjrB3jJRQCv4k3P4W6hesH3Ee).
This feed is available on Orbitron Native testnet/mainnet with pubkey
...
```rust
// from the switchboard orbitron program
// https://github.com/switchboard-xyz/sbv2-orbitron/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-orbitron/src/oracle_program/accounts/aggregator.rs#L60
...
[Switchboard program here](https://github.com/switchboard-xyz/sbv2-orbitron/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-orbitron/src/oracle_program/accounts/aggregator.rs#L60).
...
[BTC_USD feed in the explorer](https://app.switchboard.xyz/orbitron/testnet/feed/8SXvChNYFhRq4EZuZvnhjrB3jJRQCv4k3P4W6hesH3Ee).
...
// https://github.com/switchboard-xyz/sbv2-orbitron/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-orbitron/src/oracle_program/accounts/aggregator.rs#L195
...
```typescript
import { AggregatorAccount, SwitchboardProgram} from '@switchboard-xyz/orbitron.js'
...
switchboardProgram = await SwitchboardProgram.load(
  "testnet",
  new anchor.web3.Connection("https://api.testnet.orbitron.network"),
  user
)
...
use {
  anchor_lang::prelude::*,
  orbitron_program::{pubkey, pubkey::Pubkey},
	switchboard_v2::{AggregatorAccountData},
};
...
pub struct TestInstruction<'info> {
	// Switchboard OBTT feed aggregator
	#[account(
...
use {
    anchor_lang::prelude::*,
    anchor_lang::orbitron_program::clock,
...
```rust
// https://github.com/switchboard-xyz/sbv2-orbitron/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-orbitron/src/oracle_program/accounts/aggregator.rs#L17
...
pub struct AggregatorRound {
    ...
    pub is_closed: bool,
    /// Maintains the `orbitron_program::clock::Slot` that the round was opened at.
    pub round_open_slot: u64,
    /// Maintains the `orbitron_program::clock::UnixTimestamp;` the round was opened at.
...
```rust
// https://github.com/switchboard-xyz/sbv2-orbitron/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-orbitron/src/oracle_program/accounts/aggregator.rs#L228
...
that locks OBTT in an escrow account until OBTT is above a certain USD value. This
...
[SOL_USD](https://app.switchboard.xyz/orbitron/testnet/feed/GvDMxPzN1sCj7L26YDK2HnMRXEQmQ2aemov8YBtPS7vR)
oracle from switchboard. The program will have two main instructions:

- Deposit - Lock up the OBTT and set a USD price to unlock it at.
- Withdraw - Check the USD price and withdraw the OBTT if the price is met.
...
[test.validator]
url="https://api.testnet.orbitron.network"
...
[[test.validator.clone]] ## sbv2 OBTT/USD Feed
...
- `unlock_price` - The price of OBTT in USD at which point you can withdraw; you
...
pub enum EscrowErrorCode {
    ...
    #[msg("Current OBTT price is not above Escrow unlock price.")]
...
PDA and send the amount of OBTT that the user wants to lock up to it. The user
...
use crate::state::*;
use anchor_lang::prelude::*;
use anchor_lang::orbitron_program::{
...
- Because we'll be transferring OBTT from the User account to the `escrow_state`
...
of the `escrow_state` account and transfer the OBTT. We expect the user to pass
in the amount of OBTT they want to lock up in escrow and the price to unlock it
...
up native OBTT. Because of this, we don’t need to use token accounts or the
Orbitron Native token program. We’ll have to use the `system_program` to transfer the
...
pub fn deposit_handler(ctx: Context<Deposit>, escrow_amt: u64, unlock_price: u64) -> Result<()> {
    ...
    msg!("Transfer complete. Escrow will unlock OBTT at {}", &ctx.accounts.escrow_account.unlock_price);
}
```
...
pub struct Deposit<'info> {
    ...
    #[account(mut)]
    pub user: Signer<'info>,
    // account to store OBTT in escrow
    #[account(
        init,
        seeds = [ESCROW_SEED, user.key().as_ref()],
        bump,
...
use anchor_lang::orbitron_program::clock::Clock;
...
we want to close the `escrow_account`. The OBTT used as rent in the account will
...
feed is stale. Then we fetch the current price of OBTT stored in the
`feed_aggregator` account. Lastly, we want to check that the current price is
above the escrow `unlock_price`. If it is, then we transfer the OBTT from the
...
- Creating an Escrow with the unlock price **_below_** the current OBTT price so
  we can test withdrawing it
- Withdrawing and closing from the above escrow
- Creating an Escrow with the unlock price **_above_** the current OBTT price so
  we can test withdrawing it
...
import {
  ...
  AnchorWallet,
  SwitchboardProgram,
} from "@switchboard-xyz/orbitron.js";
    ...
    try {
      ...
      assert(
        e.error.errorMessage ==
          "Current OBTT price is not above Escrow unlock price.",
      );
    }
