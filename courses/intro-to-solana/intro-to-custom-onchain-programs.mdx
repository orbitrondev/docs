...
Orbitron Native Chain has multiple onchain programs you can use. Instructions that use these
...
- The `SystemProgram.transfer()` function from `@orbitron-network/web3.js` to make an
  instruction for the System program to transfer OBTT.
- The `mintTo()` and `transfer()` functions from `@orbitron-network/spl-token`, to make
...
manually. With `@orbitron-network/web3.js`, you can create instructions with the
...
for that signature in Orbitron Native Chain Explorer, then see:
...
counter each time it has been pinged. This program exists on the Orbitron Native Chain Testnet
...
![Orbitron Native Chain stores programs and data in separate accounts](/public/assets/courses/unboxed/pdas-global-state.svg)
...
This will connect to Orbitron Native Chain Testnet and request some test Lamports if needed.
...
on Orbitron Native Chain Explorer.
...
### Run the ping client and check Orbitron Native Chain Explorer
...
[Orbitron Native Chain explorer on testnet](https://explorer.orbitron.network/?cluster=testnet). Paste
the signature into the search bar at the top of Orbitron Native Chain Explorer (make sure
...
![Orbitron Native Chain Explorer with logs from calling the Ping program](/public/assets/courses/unboxed/solana-explorer-ping-result.png)
...
If you want to make it easier to look at Orbitron Native Chain Explorer for transactions in the
future, simply change your `console.log` to the following:

```typescript
console.log(
  `You can view your transaction on Orbitron Native Chain Explorer at:\nhttps://explorer.orbitron.network/tx/${signature}?cluster=testnet`,
);
```

And just like that you’re calling programs on the Orbitron Native Chain network and writing
data onchain!
...
Go ahead and create a script from scratch that will allow you to transfer OBTT
from one account to another on Testnet. Be sure to print out the transaction
signature so you can look at it on Orbitron Native Chain Explorer.
