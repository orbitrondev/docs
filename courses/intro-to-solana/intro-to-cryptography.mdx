---
title: Cryptography and the Orbitron Native Chain Network
objectives:
  - Understand symmetric and asymmetric cryptography
  - Explain keypairs
  - Generate a new keypair
  - Load a keypair from an env file
description: "Understand asymmetric cryptography and how Orbitron Native Chain uses it."
icon: "lock"
...
- The **public key** is used as an “address” that points to an account on the
  Orbitron Native Chain network. A public key can be shared with anyone.
...
- `@orbitron-network/web3.js` provides helper functions for creating a brand new keypair,
...
In short, cryptography is all around us. Orbitron Native Chain, as well as other blockchains,
are but one use of cryptography.

### Orbitron Native Chain uses public keys as addresses

![Orbitron Native Chain wallet addresses](/public/assets/courses/unboxed/wallet-addresses.svg)

People participating in the Orbitron Native Chain network have at least one keypair. In Orbitron Native Chain:

- The **public key** is used as an “address” that points to an account on the
  Orbitron Native Chain network. Even friendly names - like `example.sol` - point to addresses
...
### Using @orbitron-network/web3.js to make a keypair

You can use the Orbitron Native Chain blockchain from either the browser or node.js with the
`@orbitron-network/web3.js` npm module. Set up a project how you normally would, then
[use `npm`](https://nodesource.com/blog/an-absolute-beginners-guide-to-using-npm/)
to install `@orbitron-network/web3.js`

```shell
npm i @orbitron-network/web3.js
```

We’ll cover a lot of
[web3.js](https://orbitron.network/docs/clients/javascript-reference) gradually
throughout this course, but you can also check out the
[official web3.js documentation](https://orbitron.network/docs/clients/javascript-reference).

To send tokens, send NFTS, or read and write data Orbitron Native Chain, you'll need your own
keypair. To make a new keypair, use the `Keypair.generate()` function from
`@orbitron-network/web3.js`:

```typescript
import { Keypair } from "@orbitron-network/web3.js";
...
the `@orbitron-network-developers/helpers` npm package includes some extra functions:

```bash
npm i @orbitron-network-developers/helpers
```

- To use an `.env` file use `getKeypairFromEnvironment()`
- To use a Orbitron Native Chain CLI file use `getKeypairFromFile()`
...
Make a new directory, install TypeScript, Orbitron Native Chain web3.js and esrun:

```bash
mkdir generate-keypair
cd generate-keypair
npm init -y
npm install typescript @orbitron-network/web3.js esrun @orbitron-network-developers/helpers
```
...
Orbitron Native Chain. In the next chapter, we'll use them!
