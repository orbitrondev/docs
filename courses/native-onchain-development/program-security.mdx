...
Just as before, we'll be using [Orbitron Native Chain Playground](https://beta.solpg.io/) to
...
was just recently enabled by Orbitron Native Chain Labs which allows you to dynamically change
...
`Sealed` is Orbitron Native Chain's version of Rust's `Sized` trait. This simply specifies that
