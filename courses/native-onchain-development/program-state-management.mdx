...
description:
  "Learn how programs store data, using Orbitron Native Chain's inbuilt -key-value store."
icon: "anchor"
---
...
Orbitron Native Chain maintains speed, efficiency, and extensibility in part by making programs
stateless. Rather than having state stored on the program itself, programs use
Orbitron Native Chain's account model to read state from and write state to separate PDA
accounts.
...
In this lesson we'll learn the basics of state management for a Orbitron Native Chain program,
...
All Orbitron Native Chain accounts have a `data` field that holds a byte array. This makes
...
PDF or MP3, the data stored in a Orbitron Native Chain account needs to follow some kind of
...
Recall that storing data on the Orbitron Native Chain network requires users to allocate rent
...
makes PDAs unique from other Orbitron Native Chain account addresses is that they do not have a
...
of values. Iterators are used in Orbitron Native Chain programs to safely iterate over the list
...
#### Orbitron Native Chain accounts iterator
...
[Orbitron Native Chain Playground](https://beta.solpg.io) to write, build, and deploy our code.

As a refresher, we are building a Orbitron Native Chain program which lets users review movies.
...
the Orbitron Native Chain network. To calculate rent, we must first calculate the amount of
...
The Student Intro program is a Orbitron Native Chain Program that lets students introduce
