...
icon: "coins"
---
In this section we will discuss how you can stake using Orbitron CLI
...
```bash
orbitron-network-keygen new --no-passphrase -o stake-account.json
```
...
```bash
orbitron create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the orbitron stake-account command:

```bash
orbitron stake-account <STAKE_ACCOUNT_ADDRESS>
```

The output will look similar to this:

```bash
Total Stake: 5000 OBTT
...
Stake and withdraw authorities can be set when creating an account via the `--stake-authority` and `--withdraw-authority` options, or afterward with the `orbitron stake-authorize` command. For example, to set a new stake authority, run:

```bash
orbitron stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
orbitron create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
--stake-authority <PUBKEY> --withdraw-authority <PUBKEY> --fee-payer <KEYPAIR>
```

`<STRING>` is an arbitrary string up to 32 bytes, but will typically be a number corresponding to which derived account this is. The first account might be "0", then "1", and so on. The public key of `<STAKE_ACCOUNT_KEYPAIR>` acts as the base address. The command derives a new address from the base address and seed string. To see what stake address the command will derive, use `orbitron create-address-with-seed`:

```bash
orbitron create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to orbitron `create-stake-account`.

## Delegate Stake

To delegate your stake to a validator, you will need its vote account address. Find it by querying the cluster for the list of all validators and their vote accounts with the `orbitron validators` command:

```bash
orbitron validators
```

The first column of each row contains the validator's identity and the second is the vote account address. Choose a validator and use its vote account address in `orbitron delegate-stake`:

```bash
orbitron delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
--fee-payer <KEYPAIR>
```
...
After delegating stake, use `orbitron stake-account` to observe the changes to the stake account:
...
Delegated Stake: 4999.99771712 OBTT
...
Once delegated, you can undelegate stake with the `orbitron deactivate-stake` command:

```bash
orbitron deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
--fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `orbitron withdraw-stake` command:

```bash
orbitron withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
--fee-payer <KEYPAIR>
```
